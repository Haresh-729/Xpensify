...
const FraudReport = () => {
  ...
  return (
    <div className="p-6 w-4/5">
        ...
        <div className="w-full bg-white rounded-lg p-4 shadow-md">
          ...
          {policies.map(policy => (
            <div key={policy.id} className="mb-6 ">
              <h3 className="text-xl font-bold mb-3 text-neutral-700">{policy.name}</h3>
              <div className="grid grid-cols-2 gap-4">
              {policy.categories.map((category, index) => (
                <div key={index} className="flex items-center mb-2 p-2  rounded">
                  <div className="mr-2">
                    {category.spending <= category.budget ? (
                        <Flag className="text-green-500" fill="currentColor" size={20} />
                      ) : (
                        <Flag className="text-red-500" fill="currentColor" size={20} />
                    )}
                  </div>
                  <div className="flex-1">
                    ...
                    <div className="text-sm">
                      ...
                      Spending: <span className={category.spending > category.budget ? 'text-red-500' : 'text-green-500'}>
      ...
      {/* </div>  */}
      {/* Bottom Fraud Report */}
      <div className="bg-white rounded-lg p-4 mt-8 shadow-md">
