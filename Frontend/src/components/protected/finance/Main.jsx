...
const Main = () => {
  ...
  const fetchBills = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:3000/api/report/getlistbils",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const eventCategories = events.reduce((acc, event) => {
    // No need for conditional check here since events is initialized as []
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "created_at",
      render: (text) =>
        text ? moment(text).format("DD/MM/YYYY hh:mm A") : "-", // Handle missing timestamps
    },
  ];
